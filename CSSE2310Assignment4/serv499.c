...
Game* games; //Pointer to start of list of games
int numGames; //Number of games
Decks decks; //Struct storing the cards and number of decks
...
void new_player(Player player, char* game) {
    ...
    if (!numGames) {
        ...
        games[0].players[0] = player;
        games[0].numPlayers = 1;
        strcpy(games[0].name, game);
        numGames++;
        return;
    }
    ...
    for (int i = 0; i < numGames; i++) {
        ...
        if (!strcmp(games[i].name, game) && games[i].numPlayers != 4) {
            match = i;
            break;
        }
    }
    ...
    if (match >= 0) {
        games[match].players[games[match].numPlayers] = player;
        games[match].numPlayers++;
        if (games[match].numPlayers == 4) {
            //Start a new game with current->game
            Game startGame;
            startGame = games[match];
            pthread_create(&threadId, NULL, game_thread, (void*)&startGame);
            pthread_detach(threadId);
        }
    } else {
        //Otherwise, make a new game
        games = (Game*)realloc(games, (sizeof(Game) * (numGames + 1)));
        games[numGames].players[0] = player;
        games[numGames].numPlayers = 1;
        strcpy(games[numGames].name, game);
        numGames++;
    }
...
void * game_thread(void* arg) {
    ...
    char buffer[1024];
    Game game = *(Game*)arg;
    //Set up teams and inform clients
    ...
    //Inform clients of the winner and clean up
...
int trick(Game *game, int lead, char trumps) {
    ...
    for (int i = 0; i < 4; i++) {
        if (cmp_cards_trick(play[i], play[(i + 1) % 4], trumps, play[0].suit)
                ...
                play[0].suit) >= 0 && cmp_cards_trick(play[i], play[(i + 3) %
                4], trumps, play[0].suit) >= 0) {
